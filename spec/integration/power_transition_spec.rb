...
RSpec.describe 'PowerTransitions API', type: :request do
  ...
  path '/api/v1/guilds/{guild_id}/power_transitions' do
    ...
    post 'Creates a power transition' do
      ...
      response '422', 'invalid request' do
        ...
        let( :power_transition ) { { errors: ['Total Power can\'t be blank'] } }
        run_test!
      end
    end
  end
  ...
  path '/api/v1/guilds/{guild_id}/power_transitions/create_or_update' do
    parameter name: :guild_id, in: :path, type: :string

    patch 'Creates or Update a power transition' do
      tags 'PowerTransitions'
      consumes 'application/json'
      parameter name: :power_transition, in: :body, schema: {
